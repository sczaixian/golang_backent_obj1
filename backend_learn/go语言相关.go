...
一个目录一个包， 每个包内完成一个单一的功能高内聚，设计更小更精确，不同的包之间组合起来完成功能
首字母大小写来判断
...
使用 go 关键字创建的执行体，初始栈大小2kb，可以动态扩缩容，创建、切换开销小、内存占用低
基于信号的异步抢占式
...
----------------------- 面试题 ---------------------------------
结构体能不能比较：1. 所有字段都是可比较类型； 2. 包含不可比较字段（如 slice、map、函数），不能直接比较可可用 reflect.DeepEqual或手动比较


go 的调度器：（有3个角色：G M P，调度器是P，M是珍重执行任务的系统线程）
    啥时候工作：1.主动让出（主动放弃CPU时间片）；2.系统调用时发生阻塞，P调度下一个，等系统调用返回时会尝试获取P，如果不成功进行排队
               3.channel上读写阻塞；4.触发GC时；5.任务完成
    工作窃取：当执行队列完成了，从全局队列获取，如果全局队列为空，尝试从其他P中拿一半执行

defer语句用于延迟执行一个函数，通常用于释放资源、关闭连接、解锁等清理工作。defer语句会将函数调用推入一个栈中，当外层函数返回时，会按照后进先出（LIFO）的顺序执行这些延迟调用    
for defer：
    延迟执行，在外层函数执行结束时触发
    1.22以后循环变量在每次迭代中都是一个新的变量 fmt.Println(i) i是外层 for 中的变量时不需要(i)这样传也能捕获，不会打印多个相同值
    由于 defer 是先进后出打印
    在循环中使用defer需要小心，因为如果循环次数很多，会导致大量延迟调用被压入栈中，可能会消耗大量内存，甚至导致程序崩溃
            defer func() { fmt.Println(i) }()  修改为： func(i int) { defer fmt.Println(i)}(i) 这样每次循环为一个新的函数


select ：  用于多个 channel 发送或接收非阻塞选择，实现多路复用核心机制（监听多个channel），类似Switch，但是他的case只能是channel
            阻塞直到有一个 case 可以执行如果多个同时触发随机选一个，
            空 select 无限循环， 如果一个 case 的channel 是 nil 永远不会被选中

实现set  type Set[T comparable] struct { items map[T]struct{} }        
                func (s *Set[T]) Add(items ...T){}    
                func NewSet[T comparable]() *Set[T] {}


Go的反射包怎么找到对应的方法:   反射包（reflect）
        1. 只能获取 对外暴露 的方法


channel缓冲长度怎么决定，怎么控制上游生产速度过快
    无缓冲channel适用于强同步场景，有缓冲channel适用于允许一定速度差异的场景
    1. 对于CPU密集型任务，缓冲大小为GOMAXPROCS
    2. 对于I/O密集型任务，根据平均延迟设置缓冲
    3. 对于内存敏感场景，限制缓冲大小        


退出程序时怎么防止channel没有消费完    
    使用sync.WaitGroup来等待所有协程完成工作：确保所有协程都已经处理完数据并退出。
    使用context来通知协程退出。



内存泄漏：
无缓冲通道阻塞：如果一个goroutine在一个无缓冲通道上发送或接收数据，并且没有其他goroutine来接收或发送，那么这个goroutine就会永远阻塞。
解决方法：确保有对应的goroutine来接收或发送，或者使用带缓冲的通道。

通道未关闭：如果通道不再使用，但goroutine还在等待通道关闭，那么该goroutine可能会一直阻塞。
解决方法：在适当的时候关闭通道，或者使用上下文（context）来取消操作。

无限循环：如果goroutine中有无限循环，并且没有退出条件，那么goroutine将永远运行。
解决方法：确保循环有退出条件，或者通过上下文（context）来通知goroutine退出。

等待组使用不当：如果使用了sync.WaitGroup，但是Add和Done没有成对出现，导致Wait一直阻塞。
解决方法：确保每个Add都有对应的Done，并且在使用时注意顺序。

上下文未取消：如果goroutine使用上下文来管理生命周期，但是上下文没有被取消，那么goroutine可能会一直运行。
解决方法：使用可取消的上下文（如context.WithCancel），并在适当的时候调用取消函数。

资源未释放：例如，数据库连接、文件句柄等资源未关闭，可能导致goroutine阻塞。
解决方法：使用defer语句确保资源被释放。    