...
无状态客户端（Stateless Clients）：这是更受青睐的未来方案。
    在这个模型中，验证者（矿工/验证节点）不再需要存储完整的状态。
    相反，他们通过一种叫做 “见证数据（Witness）” 的加密证明来验证交易。
工作原理：当您发送一笔交易时，您需要附带一个小的“见证”（如Merkle证明），来证明您的账户状态和交易所需的合约状态。
    验证者只需要验证这个证明是否正确即可，而无需自己存储所有这些状态。
效果：这将极大地降低运行一个全节点的资源要求，从根本上解决状态爆炸问题。
    Vitalik Buterin（以太坊创始人）多次强调这是以太坊未来的关键升级之一。
